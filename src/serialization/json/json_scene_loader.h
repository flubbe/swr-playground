...
class JsonSceneLoader : public SceneLoader
{
public:
    void load(
      Scene& scene,
...
private:
    ...
    void load_objects(Scene& scene, simdjson::ondemand::array arr)
    {
        for(auto item: arr)
        {
            ...
            for(auto field: obj)
            {
                ...
                {
                    logging::warningf("Failed to read JSON value for key '{}': {}",
                ...
                if(key == "class")
                {
                    ...
                    {
                        throw std::runtime_error{"Object entry missing required string field 'class'."};
                    }
                }
                else
                {
            ...
            {
                throw std::runtime_error{
                  std::format("Object construction failed for class '{}'.", qualified_class_name)};
            }
    ...
    void deserialize_properties(Object& object, swr::vector<FieldEntry>& fields)
    {
        for(auto& entry: fields)
        {
            ...
            {
                logging::warningf("Unknown field '{}' found for class '{}'",
